            ...
            if (validator) {
                validator.validate().then(function (status) {
                    if (status == 'Valid') {
                        ...
                        var formData = {
                            department_id: jQuery('#department_id_update').val(),
                            name_ar: jQuery('#name_ar_update').val(),
                            name_en: jQuery('#name_en_update').val(),
                            description_ar: jQuery('#description_ar_update').val(),
                            description_en: jQuery('#description_en_update').val(),
                            active: jQuery('#select_active_update').val(),
                        };

                        var type = "PATCH";
                        var ajaxurl = '/admin/positions/'+id;
...
function getData(id) {
    ...
    var type = "GET";
    var ajaxurl = '/admin/positions/'+id+'/edit';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            jQuery('#department_id_update').val(data.department_id);
            jQuery("#department_id_update").select2("val", ""+data.department_id+"");
        },
        error: function (data) {
            Swal.fire({
                text: "معذرة ، يبدو أنه تم اكتشاف بعض الأخطاء ، يرجى المحاولة مرة أخرى.",
