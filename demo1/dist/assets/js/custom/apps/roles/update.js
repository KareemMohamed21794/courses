                ...
                validator.validate().then(function (status) {
                    if (status == 'Valid') {
                        ...
                        var formData = {
                            ...
                            permissions: jQuery('#permissions_update').val(),
                            select_active: jQuery('#select_active_update').val(),
                        };

                        var type = "PATCH";
                        var ajaxurl = '/admin/roles/'+id;
...
function getData(id) {
    ...
    var type = "GET";
    var ajaxurl = '/admin/roles/'+id+'/edit';
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function (data) {
            jQuery('#id').val(data.role.id);
            jQuery('#name_update').val(data.role.name);
            jQuery('#permissions_update').val(data.permissions);
            jQuery('#select_active_update').val(data.role.active);
            jQuery("#select_active_update").select2("val", ""+data.role.active+"");
        },
        error: function (data) {
            Swal.fire({
                text: "Sorry, looks like there are some errors detected, please try again.",
