...
var KTAccountSettingsProfileDetails = function () {
    ...
    // Private functions
    var initTaxValidation = function () {
        // Init form validation rules. For more info check the FormValidation plugin's official documentation: https://formvalidation.io/
        validationTax = FormValidation.formValidation(
            form_tax,
            {
                fields: {
                    client_id: {
    ...
    var handleFormTax = function () {
        taxSubmitButton.addEventListener('click', function (e) {
            e.preventDefault();

            // Validate form
            validationTax.validate().then(function (status) {
                if (status === 'Valid') {
                    // Show loading indication
                    ...
                    // alert('here');
                    // Send ajax request
                    axios.post(taxSubmitButton.closest('form').getAttribute('action'), new FormData(form_tax))
                        .then(function (response) {
                            // Show message popup. For more info check the plugin's official documentation: https://sweetalert2.github.io/
                            Swal.fire({
                                text: "Thank you! You've updated your basic info",
    ...
    return {
        init: function () {
            ...
            form_tax = document.getElementById('kt_tax_form');

            taxSubmitButton = form_tax.querySelector('#kt_client_tax_submit');
